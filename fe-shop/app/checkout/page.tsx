...
export default function CheckOut() {
    ...
    const updateQuantityLocally = (itemId: number, newQuantity: number) => {
        setData((prevData: any) => {
            if (!prevData || !prevData.cartItems) return prevData;
            const updatedCartItems = prevData.cartItems.map((item: any) =>
    ...
    const handleOrder = async (cartItems: {
        ...
        product_id: number;
        price: number
    }[], addressId: number, payment_id: number, note: string) => {
                                ...
                                <div>
                                    ...
                                    {data?.addresses?.length > 0 ? (
                                        ...
                                        <div>
                                            ...
                                            <a href="/user/addresses" className={"text-sm text-green-800"}>Hãy thêm địa
                                                chỉ</a>
                                        </div>
                                    )}
                                </div>
