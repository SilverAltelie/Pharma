...
export default function Show({ params}: {params: Promise<{id: string}>}) {
    ...
    useEffect (() => {
        async function fetchData() {
            ...
            const variants = data.variants.filter((variant: any) => variant.product_id == id);
            setVariants(variants || []); // Cập nhật variants
        }
        fetchData();
    }, []);
                      ...
                      >
                        ...
                        {variant.quantity > 0 ? (
                          <span
                            aria-hidden="true"
                            className={classNames(
                              selectedVariant === variant ? 'border-2 border-green-700 ' : '',
