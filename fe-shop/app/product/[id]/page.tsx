...
export default function Show({ params}: {params: Promise<{id: string}>}) {
    ...
    const [selectedVariant, setSelectedVariant] = useState<null>(null);
    ...
    async function handleAddToCart(product_id: any, variant_id: any) {
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/cart/addProductCart`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
      ...
      <div className="pt-6">
        ...
        <div className="mx-auto max-w-2xl px-4 pt-10 pb-16 sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-3 lg:grid-rows-[auto_auto_1fr] lg:gap-x-8 lg:px-8 lg:pt-16 lg:pb-24">
          ...
          <div className="mt-4 lg:row-span-3 lg:mt-0">
            ...
            <form onSubmit={handleAddToCart(product?.id, selectedVariant.id)} className="mt-10">
              {/* Colors */}
              {/* <div>
                <h3 className="text-sm font-medium text-gray-900">Color</h3>

                <fieldset aria-label="Choose a color" className="mt-4">
                <RadioGroup value={selectedVariant} onChange={setSelectedVariant} className="flex items-center gap-x-3">
                  {variants.map((variant: any) => (
                      ...
                      >
                        <span
                          aria-hidden="true"
                          className={classNames(variant.class, 'size-8 rounded-full border border-black/10')}
                        />
                      </Radio>
                    ))}
                  </RadioGroup>
              ...
              <div className="mt-10">
                ...
                <div className="mt-10">
                <p className="text-sm text-gray-900">Kho còn: {selectedVariant?.quantity}</p>
                </div>
