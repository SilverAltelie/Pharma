        ...
        async function fetchData() {
            try {
                ...
                const userAddresses = data.addresses
                    .filter((address: any) => address.user_id == data.user.id)
                    .map((addr: any) => addr.address); 

                setAddresses(userAddresses.length > 0 ? userAddresses : []);
      ...
      <div className="mt-10 ml-[200px] border-t border-gray-100">
        <dl className="divide-y divide-gray-100">
          ...
          <div className="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
            ...
            <dd className="mt-1 text-sm/10 text-gray-700 sm:col-span-2 sm:mt-0">{user.number}</dd>
          </div>
          <div className="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
